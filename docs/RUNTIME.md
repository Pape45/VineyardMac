...
This is not an activation record. The CI R2 step was skipped and the reserved beta.3 key still returned HTTP 404 immediately before the local acceptance test. On 2026-08-13, an ephemeral Swift runner importing the repository's `WhiskyKit` decoded the external plist and called the public `WhiskyWineInstaller.install(from:release:)` API on a copy of the exact archive. The installer selected the managed runtime path, installed `4.0.0-beta.3+3`, removed its input copy, reproduced the standalone manifest byte for byte, passed all 4,612 `SUMS.sha` entries, and left no staging or backup directory.

The signed Debug app then created disposable bottle `1A75B0FB-E78A-4EB4-A017-9D771436C0C4`. Its actions became available without a refresh, `Metadata.plist` recorded Wine `7.7.0`, and launching Wine Configuration through the app started `winecfg.exe` under that exact prefix. D3D11/D3D12 were not repeated because this exact candidate had already passed both isolated smokes. After the test, only that prefix's wineserver was stopped. The bottle and its reference were removed, the pre-test beta.2 runtime and empty bottle list were restored by reversible moves, and their complete file, metadata, symlink, and plist hashes matched the initial state. Public runtime `4.0.0-beta.2`, R2, and [`Runtime/RuntimeManifest.json`](../Runtime/RuntimeManifest.json) remain unchanged. Public redistribution must remain non-commercial and retain Apple's notices.
