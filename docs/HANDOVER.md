...
3. **VineyardMac integration — open but not performed.** Required files, manifest/version metadata, packaging, installer validation, and a disposable-bottle fresh setup can now be assessed without touching existing bottles or the active runtime.

Verdict: coherent GPTK 4 beta 2 build, artifact integrity, and D3D11/D3D12 device creation are demonstrated. VineyardMac integration is the next separate gate; it has not been attempted. Public redistribution must remain non-commercial and retain Apple's notices.
