...
Strategy: do not rebuild another full Whisky product surface. Keep VineyardMac focused on its real differentiator—strict runtime metadata, manifests, immutable objects, atomic replacement/rollback, and independently qualified GPTK artifacts—while collaborating with the active fork. Priorities are: (1) propose VineyardMac's strict installer and publication invariants upstream while reusing Whisky's resumable downloader; (2) reuse or jointly maintain Whisky's migration, path validation, and registry deduplication; (3) share launcher profiles, per-program backend routing, and a neutral GameDB rather than fork compatibility data; (4) reuse its diagnostic export, redaction, and troubleshooting patterns; and (5) adopt the proven Sparkle/Developer ID/notarization/Homebrew release discipline before public distribution. If a native Epic storefront becomes an explicit goal, collaborate with Mythic around Legendary instead of recreating it. Do not adopt Tequila's bundled-runtime, quarantine-removal, raw-download, or stale-update paths.
