...
[`Pape45/VineyardMac-Wine#1`](https://github.com/Pape45/VineyardMac-Wine/pull/1), `Add qualified GPTK 4 beta 2 runtime candidate`, was merged as `2de1007fd8d2752c88cc0d7f4781e43d3106eafa` after its `sdk-smoke` and complete wine64/wine32on64 build passed in [`main` CI run `31788804366`](https://github.com/Pape45/VineyardMac-Wine/actions/runs/31788804366). The executed workflow contained no R2 step. Only the exact frozen artifact set from `ff85d649d8b722f709f9b134b2d1c4eba85281cd` remains qualified; artifacts rebuilt from the merge commit are not qualified automatically.

[`Pape45/VineyardMac#6`](https://github.com/Pape45/VineyardMac/pull/6), `Document qualified GPTK 4 beta 2 candidate`, is the review record for this GPTK 4 work.
...
A read-only comparison on 2026-08-14 fixed the reviewed sources at VineyardMac `c18fe14255764ac5fe9377f9d8f1d0c99c854ba8`, [`frankea/Whisky`](https://github.com/frankea/Whisky) `4f1507944c92a1eca60ca12f0a7edaf22778aa40` (`v4.5.105-beta.1`; latest app release `app-v3.6.1`), [`MythicApp/Mythic`](https://github.com/MythicApp/Mythic) `754223978e61fb6efcff1e3152ae426bad0020b7` (latest release `v0.6.0`), and [`ThatOneTequilaDev/Tequila`](https://github.com/ThatOneTequilaDev/Tequila) `bf0bd6b5f3831978083b7f0ac21874385b75ff0a` (latest release `v1.0.0`). No code or artifact was copied into VineyardMac or executed.

- `frankea/Whisky` already covers the broad product surface: Wine 11, D3DMetal/DXVK/DXMT/WineD3D selection, per-program routing, seven launcher presets, an 83-entry GameDB, diagnostics and redacted exports, original-Whisky bottle migration, resumable runtime downloads, Sparkle, signed/notarized releases, Homebrew, dependency tracking, 94 WhiskyKit test files with 1,256 test methods, UI tests, and active issue/PR workflows. Its archive SHA-256 check is useful, but its installer still removes the live runtime before extraction and has no Vineyard-style manifest, immutable pointer protocol, or atomic rollback.
- `Mythic` owns the strongest storefront workflow: embedded Legendary integration for Epic authentication, library/install/update/repair operations, local imports, progress coordination, and internal data migrations. Its current catalog advertises Engine `2.6.1`/GPTK `2.1` stable and `3.0.0`/GPTK `3.0` preview, but uses HTTP artifact URLs; checksum URLs are modeled but not enforced, updates remove the old engine first, and the app repository has build/lint CI but no tests or documented notarization proof. It uses Sparkle, OSLog, Firebase Analytics/Crashlytics, and GPLv3, while runtime and bundled-third-party license provenance need separate review.
- `Tequila` is a thin Whisky fork claiming bundled Wine 11/GPTK plus a one-click RuntimeAIO path. It has no launcher profiles, GameDB, guided diagnostics, migration, tests, or CI; runtime downloads have no checksum validation, the app recursively removes quarantine from bundled libraries, and its Sparkle feed still targets archived Whisky infrastructure. Issues are disabled. The app is GPLv3, but runtime provenance and notices are not documented sufficiently for reuse.
- [`Sikarugir-App/Sikarugir`](https://github.com/Sikarugir-App/Sikarugir) was added as a separate reference on 2026-08-20 at `39710f11e0b9a1b4a1a7110ef8c8ad6fbf1fe786`. Its README calls it a successor to Wineskin and explicitly says it is not a replacement for Whisky or CrossOver. Its strongest evidence is operational: active Wine, DXVK, DXMT, MoltenVK, engine, wrapper, and winetricks repositories plus WineD3D, VKD3D, D3DMetal, DXMT, and DXVK choices. The main repository has no declared repository-wide license, tags, or releases and contains no inspectable application source; `Creator` likewise exposes releases without a declared source license. The separate [`Sikarugir-foss-sources`](https://github.com/Sikarugir-App/Sikarugir-foss-sources) repository at `4be1b048f8df14b073a6e39e8245bbb52c6a71c0` provides LGPL-2.1 Objective-C Wineskin configuration sources, but no visible automated tests or CI. Treat Sikarugir as a strong runtime/wrapper behavior reference, not as a wholesale application base or proof of code quality.

Strategy: remain autonomous and do not contact other repositories by default. Evaluate reuse component by component rather than accepting or rejecting an entire project for being ahead or behind: require a real VineyardMac gap, a clear compatible license, inspectable code that fits the current architecture, and a focused local test proving the behavior. When the exact license permits reuse, preserve its notices, attribution, provenance, modification record, and all copyleft obligations. When permission is ambiguous or incompatible, do not copy code, assets, text, or distinctive UI; record the behavior worth learning from and implement it independently. Citation is useful provenance but never substitutes for permission. A stale project may still contain a proven useful component, while popularity or broad feature coverage alone never proves code quality.

Keep VineyardMac focused on strict runtime metadata, manifests, immutable objects, atomic replacement/rollback, and independently qualified artifacts. Inspect the active Whisky fork first for resumable downloads, migration, path validation, registry deduplication, launcher profiles, per-program backend routing, GameDB structure, diagnostics/redaction, and release discipline; take only the smallest licensed parts that pass VineyardMac tests. Use Mythic's Legendary workflow only as a behavioral reference unless a separately licensed integration is explicitly justified. Use Sikarugir for runtime, engine, wrapper, and renderer behavior evidence, not as a front-end base. Do not adopt Tequila's bundled-runtime, quarantine-removal, raw-download, or stale-update paths.
...
`Process.runStream` now applies one redaction policy to process names in Unified Logging and to arguments and environment values in both Unified Logging and Wine `.log` files. Token, secret, password, credential, cookie, authorization, API/access/private key, and session identifiers are replaced with `<redacted>`; separate sensitive flag values, inline assignments, URL passwords, and sensitive query values are covered, while ordinary arguments and environment values remain. Embedded newlines are escaped. A focused WhiskyKit test locks those guarantees.

Launch files still record the app version, macOS version, full managed-runtime version, bottle Wine version, Windows version, sync, Metal, DXR, AVX, DXVK settings, useful command arguments, and redacted environment. Common app launch, bottle-creation, Wine-tool, Winetricks, shortcut, runtime-download/install, and maintenance failures no longer disappear through `print`: they use `Logger`, existing inline setup errors or a localized alert, and the existing Open Logs Folder action. Expected missing DPI/runtime metadata remains non-error state rather than noisy logging.
