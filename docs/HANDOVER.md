...
Because graphics bytes changed, the exact candidate was retested with the preserved DirectX executable in two fresh disposable prefixes. D3D11 exited `0` at feature level `0xb000`; D3D12 exited `0` after device creation. Neither log contains a page fault or `syscall count mismatch 232 / 233`; transient log SHA-256 values are `397dbd31632c861860a8b98fb0ae836cefaced5dc11adaa821dff065fdc59260` and `b8a774c6baaeb659f1a49e238c73ef58bf04d4e100de88ccb8106939c9c54608`. Both targeted wineservers stopped and no related process remained. The reserved beta.3 key returned HTTP 404 before and after CI; no installer, runtime, bottle, signature, or R2 object was touched. Public runtime beta.2 and repository `Runtime/RuntimeManifest.json` remain unchanged.
...
4. **VineyardMac installation — open but not performed.** Installer execution and a disposable-bottle fresh setup remain a separate future gate and must not touch existing bottles or the active runtime.

Verdict: coherent GPTK 4 beta 2 build, strict private candidate integrity, and D3D11/D3D12 device creation are demonstrated. Public runtime beta.2 remains active; beta.3 is neither published nor installed. VineyardMac installation is the next separate gate. Public redistribution must remain non-commercial and retain Apple's notices.
