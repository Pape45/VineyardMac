...
2. **D3D11/D3D12 smoke — failed at the first runtime call.** Both exact-artifact tests fail before device creation because `win32u.NtGdiDdDDIEnumAdapters2` remains a stub. The earlier hybrid-runtime page fault and syscall mismatch did not recur. Implement this missing Wine-side function, rebuild coherently, and repeat both smoke tests before proceeding.
...
Verdict: coherent GPTK 4 beta 2 build and artifact integrity are demonstrated, but the graphics smoke gate is closed by the missing `win32u.NtGdiDdDDIEnumAdapters2` implementation. Do not attempt VineyardMac runtime integration before a rebuilt artifact passes both smoke tests. Public redistribution must remain non-commercial and retain Apple's notices.
