...
final class BottleSettingsTests: XCTestCase {
    func testDecodeCreatesMissingMetadataWithDefaultWineVersion() throws {
        ...
        let created = try BottleSettings.decode(from: metadataURL)
        let decoded = try BottleSettings.decode(from: metadataURL)

        XCTAssertTrue(FileManager.default.fileExists(atPath: metadataURL.path))
        XCTAssertEqual(created.wineVersion, .init(7, 7, 0))
        XCTAssertEqual(decoded, created)
    }
}
